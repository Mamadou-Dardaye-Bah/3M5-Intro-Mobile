...
```
À chaque étape, porte une attention particulière au contexte d’exécution. Ne te limite pas à examiner le contexte de la fonction utilisée : explore également celui de la fonction appelante en remontant la pile d’appels.

Observes-tu un comportement surprenant ? Si oui, explique-le dans tes propres mots et tente d’en identifier la cause. Pour t'aider, tu peux te référer à ces explications : [Types valeur vs types référence (2P6)](https://info.cegepmontpetit.ca/2P6/cours/valeur-reference)
